...
if $DoGetData;then
#------------------------------------------------------------------------------
hadoop fs -Dhadoop.client.ugi=$hadoopclients -ls VpaOutput_guobk/userhistory_$dataSourceuserJoin0/feature/feature_user/$Date
hadoop fs -Dhadoop.client.ugi=$hadoopclients -ls VpaOutput_guobk/userhistory_$dataSource0/feature/feature_global/$Date
hadoop fs -Dhadoop.client.ugi=$hadoopclients -ls VpaOutput_guobk/userhistory_$dataSourceuserJoin1/feature/feature_user/$Date
hadoop fs -Dhadoop.client.ugi=$hadoopclients -ls VpaOutput_guobk/userhistory_$dataSource1/feature/feature_global/$Date
#------------------------------------------------------------------------------
for((u=0;u<16;u++));
do
    user=${ListUser[$u]}
    hadoop fs -Dhadoop.client.ugi=$hadoopclients -get VpaOutput_guobk/userhistory_$dataSource0/feature/feature_global/$Date/user_$user $datapath/feature_global/user_$user-and
    hadoop fs -Dhadoop.client.ugi=$hadoopclients -get VpaOutput_guobk/userhistory_$dataSource1/feature/feature_global/$Date/user_$user $datapath/feature_global/user_$user-ios
done
echo "GET user feature"
if $DoGetUserFeature;then
rm -rf /search/odin/guobk/streaming/vpa/vpa-data-process/userJoinFeature/$Date/*
for((u=0;u<16;u++));
do
    user=${ListUser[$u]}
    hadoop fs -Dhadoop.client.ugi=$hadoopclients -get VpaOutput_guobk/userhistory_$dataSourceuserJoin0/feature/feature_user/$Date/user_$user $path_user/user_$user-and
    hadoop fs -Dhadoop.client.ugi=$hadoopclients -get VpaOutput_guobk/userhistory_$dataSourceuserJoin1/feature/feature_user/$Date/user_$user $path_user/user_$user-ios
done
fi
...
do
    ...
    do
        ...
        do
            user=${ListUser[$u]}
            hadoop fs -Dhadoop.client.ugi=$hadoopclients -get VpaOutput_guobk/userhistory_$dataSource0/feature/feature_session/$Date_sess/user_$user $path_session/user_$user-and >> log/tmp/get_session.log
            hadoop fs -Dhadoop.client.ugi=$hadoopclients -get VpaOutput_guobk/userhistory_$dataSource1/feature/feature_session/$Date_sess/user_$user $path_session/user_$user-ios >> log/tmp/get_session.log
        done
        if [ "1" -ge "$epoch" ];then
        if [ "0" -ge "$day" ];then
        ...
        do
            ...
            python -u GetTrainingBatch.py $path_global $path_user $path_session $user $path_tmpdata $epoch $Date_sess $path_userPre $joining >> ./log/tmp/tmp-$user-$epoch.log 2>&1 &
        done
