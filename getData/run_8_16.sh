...
nextHour_Day=`date -d "$((24*$nDay+$curHour-$i-1)) hour ago" +"%d"`
nextHour_Hour=`date -d "$((24*$nDay+$curHour-$i-1)) hour ago" +"%H"`
#echo $lastHour_Year$lastHour_Month$lastHour_Day$lastHour_Hour,$curYear$curMonth$curDay$Hour,$nextHour_Year$nextHour_Month$nextHour_Day$nextHour_Hour
Year=$curYear
Month=$curMonth
Day=$curDay
hour=$newHour
...
hour=$nextHour_Hour
input3="/storage/sogou/desktop/imeservice/vpare/"$Year$Month"/"$Year$Month$Day"/*"$Month"-"$Day"_"$hour"*.lzo"
sh run_join.sh $input0 $input1 $input2 $input3 $curYear $curMonth $curDay $newHour >> $logfile 2>&1